...
namespace
{
    ...
    enum class WorldUpdatePhase : std::size_t
    {
        ...
        process_tc9_async_tasks,
        process_tc9_hooks,
        process_tc9_requests,
        update_metrics,
...
void World::Update(uint32 diff)
{
    ...
    {
        {
            auto observabilityPhaseTimer = MEASURE_WORLD_UPDATE_PHASE(process_tc9_async_tasks);
            METRIC_TIMER("world_update_time", METRIC_TAG("type", "Process TC9 async tasks"));
            sToCloud9Sidecar->ProcessAsyncTasks();
        }

        {
            auto observabilityPhaseTimer = MEASURE_WORLD_UPDATE_PHASE(process_tc9_hooks);
            METRIC_TIMER("world_update_time", METRIC_TAG("type", "Process TC9 hooks"));
            sToCloud9Sidecar->ProcessHooks();
        }

        {
            auto observabilityPhaseTimer = MEASURE_WORLD_UPDATE_PHASE(process_tc9_requests);
