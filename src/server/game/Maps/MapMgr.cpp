...
void MapMgr::UpdateAggregateMetrics()
{
    ...
    for (std::size_t mapId = 0; mapId < counts.size(); ++mapId)
    {
        ...
        MapCreaturesFamily.SetIndexed(mapId, "map_id", uint32(mapId), double(count->Creatures));
        MapGameObjectsFamily.SetIndexed(mapId, "map_id", uint32(mapId), double(count->GameObjects));
