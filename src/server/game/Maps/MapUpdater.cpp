...
namespace
{
    ...
    Acore::Observability::ScopedHistogramTimer MeasureMapUpdate(uint32 mapId)
    {
        // Custom DBC maps above MAX_MAP_ID have no indexed slot
        if (mapId >= MAP_ID_METRIC_COUNT)
            return {};

        return Metrics.UpdateDuration.MeasureIndexed(mapId, "map_id", mapId);
    }
}
...
public:
    ...
    void call() override
    {
        ...
        {
            Acore::Observability::ScopedHistogramTimer observabilityTimer = MeasureMapUpdate(m_map.GetId());
