...
TEST(MetricRegistryTest, DistinctLabelsProduceDistinctSeries)
{
    ...
    EXPECT_THAT(visitor.Families, testing::ElementsAre(
        RecordingVisitor::Family{ "ac_database_queue_size", "Current database async queue size",
            MetricKind::Counter }));
    EXPECT_THAT(visitor.Samples, testing::ElementsAre(
...
TEST(MetricRegistryTest, HistogramBucketsSortedAndDeduplicated)
{
    ...
    EXPECT_THAT(visitor.Samples, testing::ElementsAre(
        ...
        RecordingVisitor::Sample{ "ac_world_update_duration_seconds", "_bucket", SampleLabels{ { "le", "+Inf" } },
            0.0 },
...
TEST(MetricRegistryTest, HistogramObservationsFillCumulativeBucketsSumAndCount)
{
    MetricRegistry registry;
    Detail::HistogramSeries& series = registry.RegisterHistogram("op_seconds", "Operation duration",
        { 0.5, 1.0 }, {}, std::source_location::current());

    Detail::ObserveHistogramSeries(series, 0.25);
    Detail::ObserveHistogramSeries(series, 0.5);
    Detail::ObserveHistogramSeries(series, 0.75);
    Detail::ObserveHistogramSeries(series, 4.0);

    RecordingVisitor visitor;
    registry.VisitMetrics(visitor);

    EXPECT_THAT(visitor.Samples, testing::ElementsAre(
        RecordingVisitor::Sample{ "op_seconds", "_bucket", SampleLabels{ { "le", "0.5" } }, 2.0 },
        RecordingVisitor::Sample{ "op_seconds", "_bucket", SampleLabels{ { "le", "1" } }, 3.0 },
        RecordingVisitor::Sample{ "op_seconds", "_bucket", SampleLabels{ { "le", "+Inf" } }, 4.0 },
        RecordingVisitor::Sample{ "op_seconds", "_sum", SampleLabels{}, 5.5 },
        RecordingVisitor::Sample{ "op_seconds", "_count", SampleLabels{}, 4.0 }));
}
...
TEST(MetricRegistryTest, ConstantLabelAppearsOnCounterGaugeAndHistogramSeries)
{
    ...
    SampleLabels const realmLabels{ { "realm", "test" } };
    EXPECT_THAT(visitor.Samples, testing::ElementsAre(
        RecordingVisitor::Sample{ "ac_world_online_players", "", realmLabels, 0.0 },
        RecordingVisitor::Sample{ "ac_world_packets_total", "", realmLabels, 0.0 },
        RecordingVisitor::Sample{ "ac_world_update_duration_seconds", "_bucket",
            SampleLabels{ { "realm", "test" }, { "le", "0.1" } }, 0.0 },
        RecordingVisitor::Sample{ "ac_world_update_duration_seconds", "_bucket",
            SampleLabels{ { "realm", "test" }, { "le", "+Inf" } }, 0.0 },
        RecordingVisitor::Sample{ "ac_world_update_duration_seconds", "_sum", realmLabels, 0.0 },
        RecordingVisitor::Sample{ "ac_world_update_duration_seconds", "_count", realmLabels, 0.0 }));
}
