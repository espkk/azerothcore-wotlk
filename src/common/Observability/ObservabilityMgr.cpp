...
namespace Acore::Observability
{
    namespace
    {
        ...
        PrometheusEndpointConfig LoadPrometheusEndpointConfig()
        {
            PrometheusEndpointConfig config;
            config.Enabled = sConfigMgr->GetOption<bool>("Observability.Prometheus.Enable", true);
    ...
    void ObservabilityMgr::LoadFromConfigs()
    {
        ...
        PrometheusEndpointConfig config = LoadPrometheusEndpointConfig();
        if (!config.Enabled)
        {
            _prometheusExporter.reset();
            return;
        ...
        _prometheusExporter->ApplyConfig(std::move(config));
    }
