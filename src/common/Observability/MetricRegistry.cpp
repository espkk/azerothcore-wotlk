...
namespace Acore::Observability
{
    namespace
    {
        ...
        std::vector<BucketBoundary> MakeBucketBoundaries(Buckets buckets)
        {
            ...
            for (double bucket : buckets)
                boundaries.push_back({ bucket, Acore::StringFormat("{}", bucket) });

            return boundaries;
        }

        // std::atomic<double>::fetch_add needs libc++ 18, which Apple clang does not ship yet
        void AtomicAdd(std::atomic<double>& target, double amount)
        {
            double expected = target.load(std::memory_order_relaxed);
            while (!target.compare_exchange_weak(expected, expected + amount, std::memory_order_relaxed))
            {
            }
        }
    }
    ...
    namespace Detail
    {
        struct CounterSeries
        {
            ...
            void VisitSample(MetricFamilyView family, std::vector<Label> const& constantLabels,
                MetricVisitor& visitor) const
            {
                LabelSetView labels{ std::span{ constantLabels }, std::span{ Labels }, {} };
                visitor.VisitSample(family, { "", labels, double(Value.load(std::memory_order_relaxed)) });
            }
        ...
        struct GaugeSeries
        {
            ...
            {
                LabelSetView labels{ std::span{ constantLabels }, std::span{ Labels }, {} };
                visitor.VisitSample(family, { "", labels, Value.load(std::memory_order_relaxed) });
            }
        ...
        struct HistogramSeries
        {
            ...
            void VisitSamples(MetricFamilyView family, std::vector<Label> const& constantLabels,
                MetricVisitor& visitor) const
            {
                ...
                for (std::size_t i = 0; i < Buckets.size(); ++i)
                {
                    ...
                    LabelView leLabel[] = { { "le", Buckets[i].LeLabel } };
                    LabelSetView bucketLabels{ std::span{ constantLabels }, std::span{ Labels }, leLabel };
                    visitor.VisitSample(family, { "_bucket", bucketLabels, double(cumulativeCount) });
                }

                uint64 count = Count.load(std::memory_order_relaxed);
                LabelView infLabel[] = { { "le", "+Inf" } };
                LabelSetView infLabels{ std::span{ constantLabels }, std::span{ Labels }, infLabel };
                LabelSetView labels{ std::span{ constantLabels }, std::span{ Labels }, {} };
                visitor.VisitSample(family, { "_bucket", infLabels, double(count) });
                visitor.VisitSample(family, { "_sum", labels, Sum.load(std::memory_order_relaxed) });
                visitor.VisitSample(family, { "_count", labels, double(count) });
            }
        ...
        void ObserveHistogramSeries(HistogramSeries& series, double value)
        {
    ...
    struct MetricRegistry::Entry
    {
        ...
        Entry(StaticStringLiteral name, StaticStringLiteral help, std::source_location owner,
            std::vector<BucketBoundary> buckets, std::size_t indexedSeriesCount)
            : Name(name), Help(help), Owner(owner),
              Data(std::in_place_type<HistogramEntry>, std::move(buckets), indexedSeriesCount)
        {
        }
    ...
    Counter::Counter(StaticStringLiteral name, StaticStringLiteral help, std::vector<Label> labels,
        std::source_location owner)
    ...
    Gauge::Gauge(StaticStringLiteral name, StaticStringLiteral help, std::vector<Label> labels,
        std::source_location owner)
    ...
    void Gauge::Increment(double amount) const
    {
        ...
        AtomicAdd(_series->Value, amount);
    }
    ...
    Info::Info(StaticStringLiteral name, StaticStringLiteral help, std::vector<Label> labels,
        std::source_location owner)
    ...
    Histogram::Histogram(StaticStringLiteral name, StaticStringLiteral help, Buckets buckets,
        std::vector<Label> labels, std::source_location owner)
        : _series(&sObservability->Registry().RegisterHistogram(name, help, std::move(buckets), std::move(labels),
            owner))
    {
    }
    ...
    void Histogram::Observe(double value) const
    {
        ...
        Detail::ObserveHistogramSeries(*_series, value);
    }
    ...
    ScopedHistogramTimer& ScopedHistogramTimer::operator=(ScopedHistogramTimer&& other) noexcept
    {
        ...
        if (_series)
        {
            ...
            Detail::ObserveHistogramSeries(*_series, elapsed.count());
        }
    ...
    Detail::CounterSeries& MetricRegistry::RegisterCounter(StaticStringLiteral name, StaticStringLiteral help,
        std::vector<Label> labels, std::source_location owner)
    {
    ...
    Detail::GaugeSeries& MetricRegistry::RegisterGauge(StaticStringLiteral name, StaticStringLiteral help,
        std::vector<Label> labels, std::source_location owner)
    {
        std::lock_guard<std::mutex> lock(_registryMutex);
        Entry* existing = FindExisting(name);
    ...
    Detail::GaugeSeries& MetricRegistry::RegisterInfo(StaticStringLiteral name, StaticStringLiteral help,
        std::vector<Label> labels, std::source_location owner)
    {
        std::lock_guard<std::mutex> lock(_registryMutex);
        Entry* existing = FindExisting(name);
    ...
    Detail::HistogramSeries& MetricRegistry::RegisterHistogram(StaticStringLiteral name, StaticStringLiteral help,
        Buckets buckets, std::vector<Label> labels, std::source_location owner)
    {
    ...
    MetricRegistry::Entry* MetricRegistry::RegisterHistogramFamily(StaticStringLiteral name, StaticStringLiteral help,
        Buckets buckets, std::source_location owner, std::size_t indexedSeriesCount)
    {
        std::vector<BucketBoundary> boundaries = MakeBucketBoundaries(std::move(buckets));
        ...
        Entry& entry = existing
            ? *existing
            : CreateHistogramEntry(name, help, std::move(boundaries), owner, indexedSeriesCount);
        ...
        if (histogram.IndexedSeries.size() != indexedSeriesCount)
        {
            ...
            ASSERT(false, "Incompatible observability metric registration for '{}': indexed series cache size differs",
                name);
            return nullptr;
        }
    ...
    {
        auto& histogram = std::get<HistogramEntry>(entry.Data);
        ASSERT(index < histogram.IndexedSeries.size(), "Indexed series {} out of bounds for '{}' ({} slots)",
            index, entry.Name, histogram.IndexedSeries.size());
        return histogram.IndexedSeries[index].load(std::memory_order_acquire);
    }

    Detail::HistogramSeries& MetricRegistry::RegisterIndexedHistogramSeries(Entry& entry, std::size_t index,
        std::vector<Label> labels)
    {
        auto& histogram = std::get<HistogramEntry>(entry.Data);
        ASSERT(index < histogram.IndexedSeries.size(), "Indexed series {} out of bounds for '{}' ({} slots)",
    ...
    MetricRegistry::Entry* MetricRegistry::RegisterGaugeFamily(StaticStringLiteral name, StaticStringLiteral help,
    ...
    {
        auto& gauge = std::get<GaugeEntry>(entry.Data);
        ASSERT(index < gauge.IndexedSeries.size(), "Indexed series {} out of bounds for '{}' ({} slots)",
            index, entry.Name, gauge.IndexedSeries.size());
        return gauge.IndexedSeries[index].load(std::memory_order_acquire);
    }

    Detail::GaugeSeries& MetricRegistry::RegisterIndexedGaugeSeries(Entry& entry, std::size_t index,
        std::vector<Label> labels)
    {
        auto& gauge = std::get<GaugeEntry>(entry.Data);
        ASSERT(index < gauge.IndexedSeries.size(), "Indexed series {} out of bounds for '{}' ({} slots)",
    ...
    MetricRegistry::Entry& MetricRegistry::CreateEntry(StaticStringLiteral name, StaticStringLiteral help,
        MetricKind kind, std::source_location owner, std::size_t indexedSeriesCount)
    {
        auto it = _entries.emplace(name, std::make_unique<Entry>(name, help, owner, kind, indexedSeriesCount)).first;
        return *it->second;
    }

    MetricRegistry::Entry& MetricRegistry::CreateHistogramEntry(StaticStringLiteral name, StaticStringLiteral help,
        std::vector<BucketBoundary> buckets, std::source_location owner, std::size_t indexedSeriesCount)
    {
        auto entry = std::make_unique<Entry>(name, help, owner, std::move(buckets), indexedSeriesCount);
        auto it = _entries.emplace(name, std::move(entry)).first;
        return *it->second;
    }
    ...
    bool MetricRegistry::IsCompatibleEntry(Entry const& entry, StaticStringLiteral name, StaticStringLiteral help,
        MetricKind kind, std::source_location owner) const
    {
    ...
    void MetricRegistry::LogIncompatibleRegistration(StaticStringLiteral name, std::source_location existingOwner,
        std::source_location duplicateOwner, std::string_view reason) const
    {
        LOG_ERROR("observability",
            "Incompatible observability metric '{}' ignored: {}. "
            "Existing owner: {}:{} ({}). Duplicate owner: {}:{} ({}).",
            name, reason, existingOwner.file_name(), existingOwner.line(), existingOwner.function_name(),
            duplicateOwner.file_name(), duplicateOwner.line(), duplicateOwner.function_name());
    }

    HistogramFamily::HistogramFamily(StaticStringLiteral name, StaticStringLiteral help, Buckets buckets,
        std::size_t indexedSeriesCount, std::source_location owner)
        : _entry(sObservability->Registry().RegisterHistogramFamily(name, help, std::move(buckets), owner,
            indexedSeriesCount))
    {
    }

    ScopedHistogramTimer HistogramFamily::MeasureIndexed(std::size_t index, StaticStringLiteral labelName,
        uint32 labelValue) const
    {
        ...
        Detail::HistogramSeries& series = registry.RegisterIndexedHistogramSeries(*_entry, index,
            { { labelName, std::to_string(labelValue) } });
        return ScopedHistogramTimer(series);
    }
        ...
        StaticStringLiteral labelValue) const
    {
        ...
        Detail::HistogramSeries& series = registry.RegisterIndexedHistogramSeries(*_entry, index,
            { { labelName, std::string(labelValue) } });
        return ScopedHistogramTimer(series);
    }

    GaugeFamily::GaugeFamily(StaticStringLiteral name, StaticStringLiteral help, std::size_t indexedSeriesCount,
        std::source_location owner)
        : _entry(sObservability->Registry().RegisterGaugeFamily(name, help, owner, indexedSeriesCount))
    {
    }

    void GaugeFamily::SetIndexed(std::size_t index, StaticStringLiteral labelName, uint32 labelValue,
        double value) const
    {
        if (!_entry || !IsEnabled())
            return;
        ...
        if (!series) [[unlikely]]
            series = &registry.RegisterIndexedGaugeSeries(*_entry, index,
                { { labelName, std::to_string(labelValue) } });

        series->Value.store(value, std::memory_order_relaxed);
    }

    void GaugeFamily::SetIndexed(std::size_t index, StaticStringLiteral labelName, StaticStringLiteral labelValue,
        double value) const
    {
        if (!_entry || !IsEnabled())
            return;
