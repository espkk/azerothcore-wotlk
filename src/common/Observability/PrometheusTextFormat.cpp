    ...
    namespace
    {
        ...
        public:
            ...
            void VisitFamily(MetricFamilyView family) override
            {
                ...
                {
                    Acore::StringFormatTo(std::back_inserter(_out), "# HELP {} ", family.Name);
                    WriteEscapedHelp(_out, family.Help);
                    _out.push_back('\n');
                }

                Acore::StringFormatTo(std::back_inserter(_out), "# TYPE {} {}\n", family.Name, TypeName(family.Kind));
            }

            void VisitSample(MetricFamilyView family, MetricSampleView sample) override
            {
                _out.append(family.Name);
                _out.append(sample.Suffix);
                WriteLabels(_out, sample.Labels);
                Acore::StringFormatTo(std::back_inserter(_out), " {}\n", sample.Value);
            }
