...
namespace Acore::Observability
{
              ...
              _registry(registry)
        {
            _thread = std::thread([this]
            {
                for (;;)
                {
                    ...
                    {
                        LOG_ERROR("observability", "Prometheus metrics exporter handler failed: {}", e.what());
                    }
                }
            });
        }
    ...
    private:
        ...
        void StartOnStrand()
        {
            ...
            if (error)
            {
                LOG_ERROR("observability", "Prometheus metrics exporter failed to parse bind address '{}': {}",
                    _config.BindAddress, error.message());
                return;
            }
            ...
            if (error)
            {
                LOG_ERROR("observability", "Prometheus metrics exporter failed to open socket on {}:{}: {}",
                    endpoint.address().to_string(), endpoint.port(), error.message());
                return;
            }
            ...
            if (error)
            {
                LOG_ERROR("observability", "Prometheus metrics exporter failed to set reuse_address on {}:{}: {}",
                    endpoint.address().to_string(), endpoint.port(), error.message());
            ...
            if (error)
            {
                LOG_ERROR("observability", "Prometheus metrics exporter failed to bind {}:{}: {}",
                    endpoint.address().to_string(), endpoint.port(), error.message());
                CloseAcceptorOnStrand();
                return;
            }
            ...
            if (error)
            {
                LOG_ERROR("observability", "Prometheus metrics exporter failed to listen on {}:{}: {}",
                    endpoint.address().to_string(), endpoint.port(), error.message());
                CloseAcceptorOnStrand();
                return;
            }
            ...
            LOG_INFO("observability", "Prometheus metrics exporter listening on http://{}:{}/metrics.",
