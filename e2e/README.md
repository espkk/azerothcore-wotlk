...
| `E2E_WORLDSERVER_CONF` | unset | Path to the live `worldserver.conf`. Only `pvp/arena` reads it, to set arena ratings that no GM command exposes; those tests skip without it and restore the file on cleanup |
...
| pvp/arena | rated 2v2 queue pops for both teams; a started pair is never wider than Arena.MaxRatingDifference | P2 | covered (`TestArena_*`); rating control needs `E2E_WORLDSERVER_CONF` (no GM command sets arena team rating). **Wants an exclusive realm**: the arena season, the Arena Battlemaster event and the rating config are realm-global, and a rated bracket that already holds other teams makes the pairing unattributable | #27548 |
