...
All v1 goals listed below are fulfilled by the implementation. The only deliberate adjustment from the original draft is duplicate registration semantics: compatible same-name registrations now coalesce into one metric family, while incompatible registrations log both call sites and abort at startup.
...
- No production Grafana dashboards in v1. A minimal dashboard for local testing ships with the Docker test stack under `apps/prometheus-test/grafana`.
...
- Incompatible duplicate registrations log both call sites and abort at startup. Incompatible means the same metric name is registered with a different kind, help text, histogram bucket list, or indexed-series cache shape.
...
- Producer API lives under `Acore::Observability` with `Counter`, `Gauge`, `GaugeFamily`, `Info`, `Histogram`, `HistogramFamily`, and `ScopedHistogramTimer`.
